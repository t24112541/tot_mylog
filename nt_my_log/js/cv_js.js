...
function re_encode(key_md){
	let val_pass=md5(key_md);
	let sub=4;
	let str_pass='';
	let txt=[];
	let count=0;
...
document.location.search.replace(/\??(?:([^=]+)=([^&]*)&?)/g, function () {
    function decode(s) {
	 ...
	 $_rec[decode(arguments[1])] = decode(arguments[2]);
});
function rand_color() {
...
var weblink="http://localhost/customer_categories/";
const link="./back_end/call_func";
